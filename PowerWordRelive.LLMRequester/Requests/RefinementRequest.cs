...
internal class RefinementRequest : IRequest
{
...
#if DEBUG
            AppendRefineLog(DateTime.Now, content, new List<RefinementOperation>());
#endif
            return;
        }
    ...
    private void ApplyOperation(RefinementOperation op)
    {
        switch (op.Type)
        {
            case RefinementOperation.OperationType.Append:
                _container.Add(op.Content!);
                break;

            case RefinementOperation.OperationType.Insert:
                _container.Add(op.DisplayIndex!.Value, op.Content!);
                break;

            case RefinementOperation.OperationType.Edit:
                _container.Edit(op.DisplayIndex!.Value, op.Content!);
                break;

            case RefinementOperation.OperationType.Remove:
        ...
        IReadOnlyList<RefinementOperation> operations)
    {
        ...
        var opsLines = operations.Count == 0
            ...
            {
                RefinementOperation.OperationType.Append => $"  append: {o.Content}",
                RefinementOperation.OperationType.Insert => $"  insert @{o.DisplayIndex}: {o.Content}",
                RefinementOperation.OperationType.Edit => $"  edit @{o.DisplayIndex}: {o.Content}",
                RefinementOperation.OperationType.Remove => $"  remove @{o.DisplayIndex}",
                _ => "  ?"
            }));
