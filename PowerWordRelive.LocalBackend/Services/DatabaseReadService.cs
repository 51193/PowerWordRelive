...
public class DatabaseReadService
{
    ...
    public async Task<(List<object> items, int total)> ListTasksAsync(string status, int limit, int offset)
    {
        if (!_fs.FileExists(_dbPath))
            return (new List<object>(), 0);
        ...
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = """
                              SELECT summary, detail FROM task_entries
                              WHERE status = @status
                              ORDER BY id
                              LIMIT @limit OFFSET @offset
                          """;
        ...
        await using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
            items.Add(new { summary = reader.GetString(0), detail = reader.GetString(1) });

        var total = 0;
        await using var countCmd = conn.CreateCommand();
        countCmd.CommandText = "SELECT COUNT(*) FROM task_entries WHERE status = @status";
