...
| ffmpeg | `apt install ffmpeg` | [ffmpeg.org](https://ffmpeg.org/download.html) 下载并加入 PATH | 是（说话人分离内部使用） |

> **Windows 用户注意**：Windows 10/11 内建 WASAPI 环回，系统音频捕获无需任何额外设备或配置。
...
|------|------|
| Linux (x64) | 正式支持，充分测试 |
| Windows (x64) | 支持（WASAPI 原生环回，无需额外设备），**缺少充分测试**。如有问题请在 [Issues](../../issues) 提报 |
| macOS | 未适配（欢迎贡献） |
