...
LogRedirector.Info("LegendLore.SpeakerSplit", "SpeakerSplit starting",
    new { inputDir, outputDir, embeddingsDir, device, matchThreshold, pollIntervalSec });

var process = new LegendLore.SpeakerSplit.SpeakerSplitProcess(
    inputDir, outputDir, embeddingsDir,
    pythonScriptPath, pythonPath, cacheRoot,
    hfToken, device, matchThreshold, pollIntervalSec, fs);

using var cts = new CancellationTokenSource();
Console.CancelKeyPress += (_, e) =>
{
    e.Cancel = true;
    LogRedirector.Info("LegendLore.SpeakerSplit", "Shutting down...");
    cts.Cancel();
};
...
LogRedirector.Info("LegendLore.SpeakerSplit", "SpeakerSplit stopped");
return 0;
